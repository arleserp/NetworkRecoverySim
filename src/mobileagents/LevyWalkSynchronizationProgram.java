...
//Manejar Grados y permutar nodos con base en los vecinos
    ...
    public Action compute(Percept p) {
        ...
        Collection<MyVertex> vs = (Collection<MyVertex>) p.getAttribute("neighbors");
        ArrayList<Integer> dir = new ArrayList();
        ArrayList<MyVertex> vertices = new ArrayList();

        //System.out.println("vs" + vs.size());
        for (int j = 0; j < vs.size(); j++) {
            //System.out.println("d"+((MyVertex) (vs.toArray()[j])).toString() + " vs lv:" + lv);
            if (vs.size() == 1 || !((MyVertex) (vs.toArray()[j])).toString().equals(lv)) {
                vertices.add((MyVertex) vs.toArray()[j]);
            }
        }
        ...
        if (par % 2 == 0) {
            lv = ((MyVertex) vertices.toArray()[dmap]).toString();
        }
