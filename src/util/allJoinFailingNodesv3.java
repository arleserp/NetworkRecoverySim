 ...
 */
public class allJoinFailingNodesv3 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int limitExp = -1;
        if (args.length >= 1) {
            limitExp = Integer.parseInt(args[0]);
        }
            ...
            if (fileSrc.isFile() && extension.equals("graph")) {
                ...
                for (File fileB : files) {
                    ...
                    if (fileB.isFile() && extension.equals("graph")) { // && fileB.getName().contains(fileA) && fileA.contains("spoke") == fileB.getName().contains("spoke")) {
                        ...
                        String sep = ",";
                        ArrayList<String> nodeTmp = new ArrayList<>();
                        ...
                        for (MyVertex v : originalNetwork.getVertices()) {
                            ...
                            if (containsVertex(B, v.getName())) {
                                contR++;
                                nodeTmp.add(v.getName() + sep + "Recovered");
                                //nodeCSVFile.println(v.getName() + sep + "Recovered");
                            } else {
                                contF++;
                                nodeTmp.add(v.getName() + sep + "Failed");
                        ...
                        if (contF > 0 && contF != cont) {
                            contExpWritten++;
                            for (String line : nodeTmp) {
                                nodeCSVFile.println(line);
                            }
                        }

                        if (limitExp != -1 && contExpWritten == limitExp){
                            System.out.println("Limit of expected experiments reached. Process has ended succesfully.");
                            break;
                        }
                ...
                System.out.println(contExpWritten + " experiments joined.");
                nodeCSVFile.close();
                // edgeCSVFile.close();
            }
        }
